...
# 配置日志记录
LOG_FILE = Path(__file__).resolve().parent / "clientChatTest.log"
logging.basicConfig(
    level=logging.INFO,
...
class Configuration:
    ...
    def __init__(self) -> None:
        ...
        self.load_env()
        self.base_url = os.getenv("CHAT_API_URL", os.getenv("LLM_API_URL"))
        self.api_key = os.getenv("CHAT_API_KEY", os.getenv("LLM_API_KEY"))
        self.chat_model = os.getenv("CHAT_MODEL", os.getenv("LLM_CHAT_MODEL"))
    ...
    def llm_api_key(self) -> str:
        ...
        if not self.api_key:
            ...
            raise ValueError("CHAT_API_KEY not found in environment variables")
        # 返回 self.api_key 的值
        return self.api_key
    ...
    def llm_base_url(self) -> str:
        ...
        if not self.base_url:
            # 如果不存在，抛出 ValueError 异常
            raise ValueError("CHAT_API_URL not found in environment variables")
        # 返回 self.base_url 的值
    ...
    def llm_chat_model(self) -> str:
        ...
        if not self.chat_model:
            # 如果不存在，抛出 ValueError 异常
            raise ValueError("CHAT_MODEL not found in environment variables")
        # 返回 self.base_url 的值
...
class Server:
    ...
    async def initialize(self) -> None:
        ...
        try:
            # 建立 SSE 连接并保持流
            self.sse_context = streamablehttp_client(
                url=url,
...
class LLMClient:
    ...
    def get_response(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            # 解析响应为 JSON 格式的数据
            data = response.json()
            logging.info("LLM response completed in %.2fs", time.perf_counter() - start_time)
            # 从 JSON 响应中提取工具的输出内容
        ...
        # 记录错误信息，str(e) 提供异常的具体描述
            ...
            return f"{LLM_ERROR_PREFIX}{error_message}"
...
class ChatSession:
    ...
    def format_llm_error(self, response: str) -> str:
        error = response.removeprefix(LLM_ERROR_PREFIX)
        return (
            "\n模型响应失败，本轮没有继续执行工具，也没有写入对话历史。\n"
            f"原因: {error}\n"
            "你可以选择:\n"
            "- 输入 retry：重试刚才的问题\n"
            "- 重新输入一个更短或更明确的问题\n"
            "- 输入 exit / quit / ex / q：退出\n"
        )

    def extract_json_object(self, text: str) -> Optional[Dict[str, Any]]:
        """从模型输出中提取JSON对象，兼容模型在JSON前后夹杂解释文本的情况。"""
        try:
    ...
    async def process_llm_response(self, llm_response: str) -> str:
        try:
            ...
            llm_call = self.extract_json_object(llm_response)
            if not llm_call:
                return llm_response
            ...
            if "tool" in llm_call and "arguments" in llm_call:
                ...
                for server in self.servers:
                    try:
                        list_tools_start_time = time.perf_counter()
                        tools = await server.list_tools()
                        logging.info(
                            "list_tools from %s completed in %.2fs",
                            server.name,
                            time.perf_counter() - list_tools_start_time,
                        )
                        # 如果找到相应的工具
                        if any(tool.name == llm_call["tool"] for tool in tools):
                            try:
                                ...
                                logging.info(
                                    "Tool %s total processing completed in %.2fs",
                                    llm_call["tool"],
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if command in ['q', 'quit', 'ex', 'exit']:
                        logging.info("\nExiting...")
                        print("Exiting...")
                        break
                    if not user_input:
                        logging.info("空输入已忽略。请输入问题，或输入 retry 重试上一问题，exit 退出。")
                        print("空输入已忽略。请输入问题，或输入 retry 重试上一问题，exit 退出。")
                        continue
                    if command == "retry":
                        if not self.last_user_input:
                            logging.info("没有可重试的问题。请输入新问题，或输入 exit 退出。")
                            print("没有可重试的问题。请输入新问题，或输入 exit 退出。")
                            continue
                        user_input = self.last_user_input
                        logging.info("正在重试上一问题: %s", user_input)
                        print(f"正在重试上一问题: {user_input}")
                    else:
                    ...
                    for round_index in range(3):
                        ...
                        current_response = self.llm_client.get_response(messages)
                        llm_label = "First" if round_index == 0 else f"Follow-up {round_index}"
                        logging.info(
                            "%s LLM call completed in %.2fs",
                            llm_label,
                            time.perf_counter() - llm_start_time,
                        )
                        if current_response.startswith(LLM_ERROR_PREFIX):
                            messages.pop()
                            error_text = self.format_llm_error(current_response)
                            logging.error(error_text)
                            print(error_text)
                            break
                        logging.info("\nAssistant: %s", current_response)
                        print(f"\nAssistant: {current_response}")

                        tool_phase_start_time = time.perf_counter()
                        result = await self.process_llm_response(current_response)
                        logging.info(
                            "Tool/resource phase completed in %.2fs",
                            time.perf_counter() - tool_phase_start_time,
                        )

                        if result == current_response:
                        ...
                        logging.warning("Reached max tool rounds; returning last model response.")
                        messages.append({"role": "assistant", "content": current_response})

                    logging.info("Turn completed in %.2fs", time.perf_counter() - turn_start_time)
                    print(f"\n本轮完成，用时 {time.perf_counter() - turn_start_time:.2f}s")
