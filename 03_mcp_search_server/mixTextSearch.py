...
class MilvusFilterExpressionGenerator:
    ...
    def __init__(self, chat_client: OpenAI):
        """
        初始化过滤表达式生成器

        Args:
            chat_client: 对话模型客户端实例
        """
        ...
        self.schema_info = {
            "fields": {
                "docId": {"type": "VARCHAR", "max_length": 100, "description": "文档唯一标识"},
    ...
    def _normalize_date(self, date_text: str) -> str:
        """
        将常见中文/符号日期格式归一化为 Milvus 中 pubDate 可比较的 yyyy.mm.dd 格式。
        """
    ...
    def _extract_author_expression(self, user_query: str) -> str:
        """从常见说法中提取发布者/作者过滤条件。"""
        patterns = [
            r"(?:作者|发布者|来源|公众号)\s*(?:是|为|=|：|:)?\s*([^，,。；;\s]+)",
            r"([^，,。；;\s]+)\s*(?:发布|发表|写的|撰写)",
        ]
        for pattern in patterns:
            ...
            if match:
                ...
                author = re.sub(r"的?(?:文档|文章|内容|资料)$", "", author)
                if author and author not in {"的", "文档", "文章", "内容"}:
                    return f'pubAuthor == "{author}"'
        return ""

    def _extract_title_expression(self, user_query: str) -> str:
        """从常见说法中提取标题包含条件。"""
        match = re.search(r"标题\s*(?:包含|含有|包括|有)\s*([^，,。；;]+)", user_query)
        if match:
            keyword = match.group(1).strip().strip('"\'')
            keyword = re.sub(r"的?(?:文档|文章|内容|资料)$", "", keyword)
            if keyword:
                return f'title like "%{keyword}%"'
        return ""

    def _extract_date_expression(self, user_query: str) -> str:
        """从常见时间限制中提取发布时间过滤条件。"""
        date_patterns = [
        ...
        for pattern in date_patterns:
            dates.extend(re.findall(pattern, user_query))

        normalized_dates = [self._normalize_date(date) for date in dates]
        normalized_dates = [date for date in normalized_dates if date]
        if not normalized_dates:
            return ""

        if len(normalized_dates) >= 2 and re.search(r"到|至|之间|~|-", user_query):
            start_date, end_date = normalized_dates[0], normalized_dates[1]
            return f'pubDate >= "{start_date}" and pubDate <= "{end_date}"'

        date = normalized_dates[0]
        if re.search(r"之前|以前|前", user_query):
            return f'pubDate < "{date}"'
        if re.search(r"之后|以后|后", user_query):
            return f'pubDate > "{date}"'
        if re.search(r"当天|当日|这天|这一天", user_query):
            return f'pubDate like "{date}%"'
        if re.search(r"发布|发表|时间|日期", user_query):
            return f'pubDate like "{date}%"'
        return ""

    def _generate_rule_based_filter_expression(self, user_query: str) -> str:
        """
        对常见简单过滤条件做本地规则解析，避免每次都调用对话模型。
        """
        expressions = [
            self._extract_date_expression(user_query),
            self._extract_author_expression(user_query),
            self._extract_title_expression(user_query),
        ]
        ...
        if self._validate_filter_expression(filter_expr):
            logger.info(f"规则解析生成过滤表达式: {filter_expr}")
            return filter_expr
        logger.warning(f"规则解析生成的过滤表达式无效: {filter_expr}")
        return ""
...
请只返回过滤表达式，不要包含其他解释。如果无法生成有效表达式，返回空字符串。"""
    ...
    def generate_filter_expression(self, user_query: str, max_retries: int = 3) -> str:
        ...
        rule_based_expr = self._generate_rule_based_filter_expression(user_query)
        if rule_based_expr:
            return rule_based_expr
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = self.chat_client.chat.completions.create(
                    model=os.getenv("CHAT_MODEL", os.getenv("LLM_CHAT_MODEL", "gpt-4o-mini")),
                    messages=[
...
class MilvusSearchManager:
    ...
    def __init__(self,
        ...
        # 初始化客户端
        self._init_clients(chat_base_url, chat_api_key, embedding_base_url, embedding_api_key)
    ...
    def _init_clients(
        ...
        try:
            ...
            if not chat_api_key or not isinstance(chat_api_key, str):
                raise ValueError("对话模型API密钥不能为空且必须是字符串类型")

            if not embedding_api_key or not isinstance(embedding_api_key, str):
                raise ValueError("嵌入模型API密钥不能为空且必须是字符串类型")
            ...
            logger.info("嵌入模型客户端初始化成功")

            # 初始化过滤表达式生成器
            self.filter_generator = MilvusFilterExpressionGenerator(self.chat_client)
    ...
    def close(self) -> None:
        """关闭底层客户端连接，避免长时间运行时连接资源堆积。"""
        if self.milvus_client is not None:
            try:
                self.milvus_client.close()
                logger.info("Milvus客户端连接已关闭")
            except Exception as e:
                logger.warning(f"关闭Milvus客户端连接失败: {e}")
            finally:
                self.milvus_client = None

        for client_name in ("chat_client", "embedding_client"):
            client = getattr(self, client_name, None)
            if client is not None and hasattr(client, "close"):
                try:
                    client.close()
                    logger.info(f"{client_name} 已关闭")
                except Exception as e:
                    logger.warning(f"关闭 {client_name} 失败: {e}")
                finally:
                    setattr(self, client_name, None)

    def emb_text(self, text: str, model: Optional[str] = None, max_retries: int = 3) -> List[float]:
        """
        生成文本的向量嵌入
        ...
        if not text or not isinstance(text, str):
            ...
            embedding_dim = int(os.getenv("EMBEDDING_DIM", os.getenv("LLM_EMBEDDING_DIM", "1024")))
            return [0.0] * embedding_dim  # 返回默认维度的零向量

        if model is None:
            model = os.getenv("EMBEDDING_MODEL", os.getenv("LLM_EMBEDDING_MODEL", "BAAI/bge-m3"))
    ...
    def _perform_sparse_search(self, collection_name: str, query_text: str,
        ...
        try:
            ...
            logger.info(
                "稀疏向量搜索完成，返回 %s 个结果，耗时=%.2fs",
                len(res[0]) if res else 0,
                time.perf_counter() - sparse_start_time,
            )
            return res
    ...
    def _perform_dense_search(self, collection_name: str, query_text: str,
        ...
        try:
            ...
            logger.info(
                "密集向量搜索完成，返回 %s 个结果，耗时=%.2fs",
                len(res[0]) if res else 0,
                time.perf_counter() - dense_start_time,
            )
            return res
    ...
    def search_documents(self,
        ...
        try:
            ...
            if filter_query != "##None##":
                ...
                logger.info("未提供过滤条件，跳过过滤表达式生成")

            output_fields = ["title", "content_chunk", "link", "pubAuthor", "pubDate"]

            if search_type == "sparse":
                # 稀疏向量搜索（BM25全文搜索）
                result = self._perform_sparse_search(collection_name, query_text, limit, output_fields, filter_expr)
                logger.info("search_documents总耗时: %.2fs", time.perf_counter() - search_start_time)
                return result
                ...
                result = self._perform_dense_search(collection_name, query_text, limit, output_fields, filter_expr)
                logger.info("search_documents总耗时: %.2fs", time.perf_counter() - search_start_time)
                return result
